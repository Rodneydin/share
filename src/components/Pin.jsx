...
const Pin = ({pin}) => {
  ...
  return (
    <div className='m-2 '>
        <div
         className='relative cursor-zoom-in w-auto rounder-lg overflow-hidden transition-all duration-500 ease-in-out hover:shadow-lg m-0 p-0 '
         onMouseEnter={() => setPostHovered(true)}
         onMouseLeave={() => setPostHovered(false)}
         onClick={() => navigate(`/pin-detail/${_id}`)}
        >
            <img src={(urlFor(image).width(350).url())} alt="user-post" className='rounded-lg m-0 p-0' />
            {postHovered && (
                <div 
                 className='absolute top-0 w-full h-full flex-col justify-between p-1 pt-2 pb-2  z-50'
                 style={{height:'100%'}}
                 >
                  <div className='flex items-center justify-between'>
                      ...
                      {alreadySaved ? (
                        ...
                        className='bg-red-500 opacity-70 hover:opacity-100 font-bold px-5 py-1 text-base rounded-3xl hover:shadow-md outline-none mr-1'
                        > Save</button>
                      )}
                  </div>
                  <div className='flex justify-between items-center gap-2 w-full'>
                        ...
                        {postedBy._id === user.uid && (
                          <button 
                            ...
                            className="absolute right-2 bottom-4   bg-white p-2 rounded-full w-8 h-8 flex items-center justify-center text-dark opacity-75 hover:opacity-100 outline-none "
                          >
                            <AiTwotoneDelete />
                          </button>
                        )}
